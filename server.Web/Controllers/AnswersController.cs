...
public class AnswersController : ControllerBase
{
  ...
  public async Task<IActionResult> AddAnswer([FromBody] AddedAnswerDto addedAnswer,
    [FromServices] IValidator<AddedAnswerDto> addedAnswerValidator)
  {
    ValidationResult addedAnswerValidatorResult = 
      addedAnswerValidator.Validate(addedAnswer);
  ...
  public async Task<IActionResult> ChangeAnswer(int id, 
    [FromBody] ChangedAnswerDto changedAnswer,
    [FromServices] IValidator<ChangedAnswerDto> changedAnswerDtoValidator)
  {
    ValidationResult changedAnswerDtoValidatorResult = 
      changedAnswerDtoValidator.Validate(changedAnswer);

    if (!changedAnswerDtoValidatorResult.IsValid)
      return BadRequest(new Response(changedAnswerDtoValidatorResult.Errors.First().ErrorMessage));
  ...
  public IEnumerable<AnswerDto> GetAllAnswers(int questionId) =>
    _answersService.GetAllAnswersByQuestionId(questionId)
