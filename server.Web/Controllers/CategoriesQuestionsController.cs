...
public class CategoriesQuestionsController : ControllerBase
{
    ...
    [FromServices] IValidator<AddedCategoryQuestionDto> addedCategoryQuestionValidator)
  {
    ...
    if (!addedCategoryQuestionValidatorResult.IsValid)
      return BadRequest(new Response(addedCategoryQuestionValidatorResult.Errors.First().ErrorMessage));

    if (await _categoriesQuestionsService.GetCategoryQuestionAsync(addedCategoryQuestion.Title) != null)
    ...
    return Ok(new CategoryQuestionDto()
      {
        Id = categoryQuestion.Id,
        Title = categoryQuestion.Title
      });
  }
  ...
  public async Task<IActionResult> ChangeCategoryQuestion(int id,
    ...
    [FromServices] IValidator<ChangedCategoryQuestionDto> changedCategoryQuestionValidator)
  {
    ...
    if (!changedCategoryQuestionValidatorResult.IsValid)
      return BadRequest(new Response(changedCategoryQuestionValidatorResult.Errors.First().ErrorMessage));
    ...
    await _categoriesQuestionsService.ChangeCategoryQuestionAsync(
      categoryQuestion, changedCategoryQuestion);
  ...
  public IEnumerable<CategoryQuestionDto> GetAllCategoriesQuestions() =>
    _categoriesQuestionsService.GetAllCategoriesQuestions()
      .Select(cq => new CategoryQuestionDto()
      {
        Id = cq.Id,
        Title = cq.Title
