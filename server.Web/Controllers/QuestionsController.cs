...
public class QuestionsController : ControllerBase
{
    ...
    {
        ValidationResult addedQuestionValidatorResult = addedQuestionValidator.Validate(
            addedQuestion
        );

        if (!addedQuestionValidatorResult.IsValid)
            return BadRequest(
                new Response(addedQuestionValidatorResult.Errors.First().ErrorMessage)
            );

        if (
            await _categoriesQuestionsService.GetCategoryQuestionAsync(
                addedQuestion.CategoryQuestionId
            ) == null
        )
    ...
    {
        ValidationResult changedQuestionValidatorResult = changedQuestionValidator.Validate(
            changedQuestion
        );

        if (!changedQuestionValidatorResult.IsValid)
            return BadRequest(
                new Response(changedQuestionValidatorResult.Errors.First().ErrorMessage)
            );

        if (
            await _categoriesQuestionsService.GetCategoryQuestionAsync(
                changedQuestion.CategoryQuestionId
            ) == null
        )
            return BadRequest(new Response("Категория с данным идентификатором не найдена"));
    ...
    )
    {
        ...
        Response.Headers.Add("x-total-count", countOfQuestions.ToString());

        return _questionsService
            .GetRangeOfQuestions(limit, page)
            .Select(
                q =>
                    new QuestionDto()
                    {
